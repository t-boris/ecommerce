...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, userID string) error {
	...
	results, err := userCollection.Aggregate(ctx, mongo.Pipeline{unwind, grouping})
	ctx.Done()
	if err != nil {
		panic(err)
	}
	...
	if err := results.All(ctx, &getUserCart); err != nil {
		panic(err)
	}
...
func InstantBuyer(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, userID string) error {
	...
	ordersDetail.ID = primitive.NewObjectID()
	ordersDetail.OrderedAt = time.Now()
	ordersDetail.Cart = make([]models.ProductUser, 0)
